...
export default function TranscriptsPage() {
  ...
  const updateTranscript = useTranscriptsStore((state) => state.updateTranscript);
  const deleteTranscript = useTranscriptsStore((state) => state.deleteTranscript);
  const restoreTranscript = useTranscriptsStore((state) => state.restoreTranscript);
  ...
  const handleDeleteClick = async (item: TranscriptData) => {
    const ok = await confirmService.confirm({ title: '削除の確認', message: `${item.year}年度「${item.course_name}」を削除しますか？`, okLabel: '削除', cancelLabel: 'キャンセル' });
    if (!ok) return;
    ...
    if (result.success) {
      ...
      toastService.error({ title: '削除失敗', description: `削除に失敗しました。再度お試しください。 (${result.error ?? 'エラー'})` });
    }
  };

  const [filterText, setFilterText] = useState('');
  const courseAbbrMap = useTranscriptsStore(state => state.courseAbbrMap);
  const availableYearsForTranscripts = useSettingsStore(state => state.availableYearsForTranscripts);
  ...
  const acquiredCredits = useMemo(() => {
    return filteredTranscripts.reduce((sum, r) => (r.status === '修得' ? sum + (r.credits ?? 0) : sum), 0);
  }, [filteredTranscripts]);
          ...
          <Table>
            <TableHeader>
              <TableRow>
                <TableHead className="w-16 text-center">
                  <div className="flex items-center justify-center">
                    <div className="relative w-full flex items-center justify-center">
                      <DropdownSelect
                        items={['年度', ...(availableYearsForTranscripts || []).map(String)]}
                        currentItem={yearFilter ?? '年度'}
                        onItemSelect={(v) => setYearFilter(v === '年度' ? null : v)}
                ...
                <TableHead className="w-24 text-center">
                  <div className="flex items-center justify-center">
                    <DropdownSelect
                      ...
                      onItemSelect={(v) => setStatusFilter(v === '履修状況' ? null : v)}
                      renderItem={(v) => v}
                      placeholder="履修状況"
                      allowReselect={true}
                    />
                ...
                <TableHead className="w-20 text-center">単位数（修得: {acquiredCredits}）</TableHead>
              ...
              {filteredTranscripts.length > 0 ? (
                ...
                filteredTranscripts.map((item) => (
                  <TableRow key={item.id} className="hover:bg-muted/50">
                    <TableCell className="w-16 text-center">{item.year}</TableCell>
                    <TableCell className="w-36 text-left">{item.course_name}</TableCell>
                    <TableCell className="w-24 text-center">{item.status}</TableCell>
                    <TableCell className="w-20 text-center">{item.credits}</TableCell>
                ...
                <TableRow>
                  ...
                  <TableCell className="w-36 text-left text-muted-foreground py-8">
                    {transcripts.length === 0 ? '今までに履修 / 修得した科目を追加してください。' : '該当する履修記録がありません。'}
                  </TableCell>
