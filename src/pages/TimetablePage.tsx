...
import { ToolsTop, Timetable, CourseList, CourseDetail } from '@/components';

import { useSettingsStore, useTimetableStore, useCellStateStore, useTranscriptsStore, useCourseSearchStore } from '@/stores';
import type { CourseListEntry, CellData, CourseData, Timetable as TimetableMatrix, CourseDetailData } from '@/types';

type SelectedCell = CellData & { day: number; period: number[] };

const hasValidCellSelection = (cell: CellData): cell is SelectedCell => {
  return cell.day !== null && Array.isArray(cell.period) && cell.period.length > 0;
};

const getCourseCodeFromCell = (cell: CellData, timetable: TimetableMatrix): string | null => {
...
export default function TimetablePage() {
  ...
  const courseEntriesByCode = useTimetableStore(state => state.courseEntriesByCode);

  const clickedCell = useCellStateStore(state => state.clickedCell);

  const transcriptStatusByCourseName = useTranscriptsStore(state => state.statusByCourseName);
  const loadTranscripts = useTranscriptsStore(state => state.loadTranscripts);
  const areTranscriptsLoaded = useTranscriptsStore(state => state.isDataLoaded);
  const requestCourseSearchAutoFill = useCourseSearchStore(state => state.requestAutoFill);

  const handleClearCourseDetail = useCallback(() => {
    setSelectedCourseCode(null);
  }, []);

  const handleShowCourseDetail = useCallback((courseCode: string) => {
  ...
  const clickedPeriodKey = clickedCell.period ? clickedCell.period.join(',') : '';
  ...
  }, [detailSelectionKey, selectedCellCourseCode, courseEntriesByCode, handleClearCourseDetail, handleShowCourseDetail]);
  ...
  const selectedCourseDetail = useMemo(() => (
    buildCourseDetail(selectedCourseCode, courseEntriesByCode, transcriptStatusByCourseName)
  ), [selectedCourseCode, courseEntriesByCode, transcriptStatusByCourseName]);

  const handleAutoFillCourseSearch = useCallback((courseName: string) => {
    if (!courseName) {
      return;
    }
  ...
  const handleRegister = async (currentCellData: CellData | null, data: CourseData[], force = false) => {
    return await register(data, currentCellData, force);
  };

  const handleYearChange = (newYear: string | number) => {
    const yearAsNumber = typeof newYear === 'string' ? parseInt(newYear, 10) : newYear;
