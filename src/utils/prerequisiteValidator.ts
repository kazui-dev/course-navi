...
import type { UserProfile, TranscriptData, PrerequisiteRuleRecord, RuleStatus, PrerequisiteViolation } from '@/types';
...
const ensureCourseSubjectMap = async (year: number): Promise<Map<string, string>> => {
...
const buildCourseSubjectMaps = async (years: number[]): Promise<Map<number, Map<string, string>>> => {
  const uniqueYears = Array.from(new Set(years.filter(year => Number.isFinite(year))));
  if (uniqueYears.length === 0) {
    return new Map();
  }
  const entries = await Promise.all(uniqueYears.map(async year => {
...
const buildTranscriptCreditMaps = (
  ...
  transcripts.forEach(record => {
    ...
    const subjectName = courseSubjectMaps.get(record.year)?.get(record.course_name);
    if (subjectName) {
...
const getStatusBucket = (map: Map<string, StatusCredits>, key: string): StatusCredits => {
  if (!map.has(key)) {
...
const isRuleApplicable = (rule: PrerequisiteRuleRecord, profile: UserProfile | null): boolean => {
...
const parseRuleNode = (raw: unknown): RuleNode | null => {
  ...
  if (node.type === 'logical') {
    ...
    const rules = Array.isArray(node.rules)
      ? node.rules.map(parseRuleNode).filter((child): child is RuleNode => Boolean(child))
      : [];
  ...
  if (node.type === 'course' || node.type === 'subject') {
    ...
    const credits = typeof node.credits === 'number'
      ? node.credits
...
const evaluateNode = (node: RuleNode, context: ValidationContext): EvaluationResult => {
  ...
  if (node.type === 'logical') {
    ...
    function buildDescriptionLines(results: EvaluationResult[], condition: LogicalCondition): string[] {
      if (condition === 'AND') {
    ...
    const descriptionLines = buildDescriptionLines(childResults, node.condition);
    const description = descriptionLines.filter(line => line.trim() !== '').join('\n');
    if (node.condition === 'AND') {
      ...
      const indicatesAlternative = childResults.some(result => result.indicatesAlternative);
  ...
  const description = node.status === '履修'
    ? `「${node.name}」`
    : `「${node.name}」${node.credits}単位修得`;
  const transcriptMap = node.type === 'course'
    ? context.transcriptCourseCredits
    : context.transcriptSubjectCredits;

  const transcriptCredits = getTranscriptCredits(transcriptMap, node.name, node.status);
...
export const validatePrerequisites = async ({
  ...
  const applicableRules = rules.filter(rule => isRuleApplicable(rule, profile));
  if (applicableRules.length === 0) {
    return null;
  }

  const filteredTranscripts = transcripts.filter(record => Number.isFinite(record.year) && record.year < year);
  const transcriptYears = Array.from(new Set(filteredTranscripts.map(record => record.year)));
  const courseSubjectMaps = await buildCourseSubjectMaps(transcriptYears);

  const transcriptCredits = buildTranscriptCreditMaps(filteredTranscripts, courseSubjectMaps);
  ...
  for (const rule of applicableRules) {
    ...
    if (!evaluation.passed) {
      return {
        messages: evaluation.missingDescriptions.length > 0
