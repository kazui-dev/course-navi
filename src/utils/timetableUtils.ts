...
export const createEmptyTimetable = (): Timetable => {
  return Array.from({ length: 12 }, () =>
    Array<TimetableCellContent>(5).fill(""),
  );
};
 ...
 */
export const convertTimetableToCodes = (
  timetable: Timetable,
): TimetableCodesTable => {
  return timetable.map((row) =>
    row.map((cell) => (typeof cell === "object" ? cell.code : "")),
  );
};
...
export const deleteCourse = (
  table: Timetable,
  courseData: TimetableCellContent,
): Timetable => {
  ...
  codesTable.forEach((row, rowIndex) => {
    row.forEach((code, colIndex) => {
      if (code) {
        ...
        if (courseDataList) {
          const matchingCourseData = courseDataList.find(
            (course) =>
...
const compareSubjectSortKeyValue = (
  a: SubjectSortKeyValue,
  b: SubjectSortKeyValue,
): number => {
...
const mapCoursesByCellKeyRecord = (
  coursesByCellMap: Map<string, Set<string>>,
): Record<string, string[]> => {
...
export function buildCourseList(rows: CourseList[]) {
  ...
  const subjectOrder = createSubjectOrderList(
    collections.subjectOrderMap,
    collections.subjectCourseMap,
  );
  const subjectCourseCodes = mapSubjectCourseCodesFromOrder(
    subjectOrder,
    collections.subjectCourseMap,
  );
  const coursesByCellKey = mapCoursesByCellKeyRecord(
    collections.coursesByCellMap,
  );
