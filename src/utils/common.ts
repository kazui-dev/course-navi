import type { ProcessedRow, UserProfile } from "@/types";
...
): { department: string; division: string; classNum: string } | null {
...
export function groupRowsByCourseCode(
  rows: RawCourseRowLocal[],
): ProcessedRow[][] {
  if (rows.length === 0) {
    return [];
  }
