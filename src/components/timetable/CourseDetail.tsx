...
function PrerequisiteList({ courseName, fallback }: { courseName: string; fallback?: string | null }) {
...
export default function CourseDetail({ detail, isLoading, onFillCourseSearch }: CourseDetailProps) {
  if (!detail && !isLoading) {
    return null;
  }
  const currentYear = useSettingsStore(state => state.currentYear);
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        const upToYear = typeof currentYear === 'number' ? currentYear : undefined;
        ...
        const courseMetadata = await dbClient.fetchCourseMetadata(upToYear ?? new Date().getFullYear());
        const acquired = useTranscriptsStore.getState().getAcquiredCredits(detail.courseName, { upToYear, includeExclusiveGroup: true, includeSameYear: false, courseMetadata });
    ...
    <Card className='w-full shadow-sm'>
      <CardHeader className='flex flex-row items-start justify-between space-y-0 pb-4'>
        <div>
          ...
          {detail && (
            <p className='text-sm text-muted-foreground mt-1'>{detail.courseName}</p>
          )}
        </div>
      </CardHeader>
          ...
          <Table>
            <TableBody>
              ...
              <TableRow>
                ...
                <TableCell>
                  <PrerequisiteList courseName={detail.courseName} fallback={detail.prerequisite} />
                </TableCell>
              </TableRow>
              <TableRow>
                ...
                <TableCell>
                  {(() => {
                    ...
                    const parts: string[] = [];
                    if (enrolledCredits > 0) parts.push(`${enrolledCredits}単位履修`);
                    if (acquiredCredits > 0) parts.push(`${acquiredCredits}単位修得`);
              ...
              <TableRow>
                <TableCell className='pl-0'>
                  <Button
                    variant='outline'
                    size='sm'
                    onClick={() => detail.courseName && onFillCourseSearch(detail.courseName)}
                  >
                    他の開講時間
                  </Button>
                </TableCell>
