...
import type { TimetableCellContent, CourseData } from '@/types';
import { useHighlightStore } from '@/stores';
import type { CellData, SearchResult } from '@/types';

const isMatch = (day: number, period: number, targetCell: CellData | SearchResult[] | CourseData[] | null): boolean => {
  if (!targetCell) return false;
  if (Array.isArray(targetCell)) {
    return targetCell.some(entry =>
      entry.day === day && entry.period.includes(period)
    );
  }
...
const TimetableCell = React.memo(
  ({
    ...
    const previewHighlight = useHighlightStore(state => state.previewHighlight);
    ...
    const cellValue = cell !== '' ? `${cell.abbr || ''} ${cell.section || ''}` : '';
    ...
    if (isEvenRow) {
      const isSame = cell !== '' && nextCell !== '' && cell.code === nextCell.code;
      ...
      return (
        <TableCell
          ...
          onClick={handleClick}
          rowSpan={isSame ? 2 : 1}
          style={{ verticalAlign: 'middle', ...(isSame ? { borderBottom: 'none' } : {}) }}
        >
          {cellValue}
        </TableCell>
      );
    ...
    const isSame = cell !== '' && prevCellTyped !== '' && cell.code === prevCellTyped.code;
