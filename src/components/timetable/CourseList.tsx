import { useEffect, Fragment, useMemo, useCallback, useState, useId } from 'react';
...
import { useTranscriptsStore, useSettingsStore, useCellStateStore, useHighlightStore, useTimetableStore } from '@/stores';
import type { CellData, CourseData } from '@/types';
import { getCellKey, formatPrereqConfirmation } from '@/utils';
...
const getCourseInfo = (data: CourseData[]): string => {
  ...
  if (credits <= 1) {
    return data.length > 1 ? `(${dayLabel()} ${singlePeriodLabel})` : `(${singlePeriodLabel})`;
  }
...
type CourseListProps = {
  register: (currentCell: CellData | null, data: CourseData[], force?: boolean) => Promise<{ success: boolean; blocked?: true; message?: string } | void>;
...
function CourseList({ register, unregister, currentYear, onShowDetail, onClearDetail, activeDetailCode = null }: CourseListProps) {
  ...
  const setPreviewHighlight = useHighlightStore(state => state.setPreviewHighlight);
  const searchHighlight = useHighlightStore(state => state.searchHighlight);
  // 検索結果のcourse_name一覧をSet化
  const highlightedNames = useMemo(() => searchHighlight ? new Set(searchHighlight.map(r => r.course_name)) : new Set(), [searchHighlight]);

  const isCourseListLoading = useTimetableStore(state => state.isCourseListLoading);

  const [hideAcquired, setHideAcquired] = useState(true);
  const [filterPrereqs, setFilterPrereqs] = useState(true);
  const allowedCoursesFromStore = useSettingsStore(state => state.currentAllowedCourses);
  const isAllowedComputedFromStore = useSettingsStore(state => state.isAllowedComputed);
  const currentClass = useSettingsStore(state => state.currentClass);
  const transcriptsVersion = useTranscriptsStore(state => state.transcriptsVersion);
  const isTranscriptsLoaded = useTranscriptsStore(state => state.isDataLoaded);
  ...
  const getVisibleCoursesForCell = useTimetableStore(state => state.getVisibleCoursesForCell);
  ...
  }, [day, periodKey, getVisibleCoursesForCell, hideAcquired, filterPrereqs, allowedCoursesFromStore, isAllowedComputedFromStore, currentYear, transcriptsVersion, currentClass]);

  function ControlsComponent(props: { filterPrereqs: boolean; setFilterPrereqs: (v: boolean) => void; hideAcquired: boolean; setHideAcquired: (v: boolean) => void }) {
    const { filterPrereqs, setFilterPrereqs, hideAcquired, setHideAcquired } = props;
    const id = useId();
      ...
      <div className="my-3">
        <div className="flex flex-col gap-2">
          <div className="flex items-center gap-2">
            <Checkbox id={filterId} checked={filterPrereqs} onCheckedChange={(v) => setFilterPrereqs(Boolean(v))} />
            <Label htmlFor={filterId}>前提条件を満たす</Label>
          </div>
          <div className="flex items-center gap-2">
            <Checkbox id={hideId} checked={hideAcquired} onCheckedChange={(v) => setHideAcquired(Boolean(v))} />
  ...
  const handleRegisterFromList = useCallback(async (data: CourseData[]) => {
    const result = await register(currentCell, data);
    if (result && typeof result === 'object' && 'blocked' in result && (result as { blocked?: true }).blocked) {
      const courseName = data[0]?.course ?? '';
      const anyRes = result as any;
      if (anyRes.confirmType === 'maxCredits' && typeof anyRes.message === 'string') {
        ...
        const ok = await confirmService.confirm({ title, message, okLabel: '登録', cancelLabel: 'キャンセル' });
        if (!ok) return;
        await register(currentCell, data, true);
        return;
      }

      const raw = 'message' in result ? (result as { message?: string }).message : undefined;
      let title: string | undefined;
      let message: string = '';
      try {
        const res = await formatPrereqConfirmation(courseName, raw, currentYear ?? undefined);
        title = res.title;
        message = res.message;
      } catch (err) {
    ...
    return (
      <div>
        <div className="mb-2">
          ...
          <ControlsComponent filterPrereqs={filterPrereqs} setFilterPrereqs={(v) => setFilterPrereqs(v)} hideAcquired={hideAcquired} setHideAcquired={(v) => setHideAcquired(v)} />
        </div>
        <Card className='p-3 text-center text-muted-foreground'>
          授業データを読み込んでいます…
        </Card>
              ...
              {courseList.map(({ subject, entries }) => (
                <Fragment key={subject}>
                  {entries.map((data, index) => {
                    ...
                    const highlightClass = highlightedNames.has(courseData.course) ? 'bg-yellow-100' : '';
                    ...
                    return (
                      <TableRow key={courseData.code}>
                        ...
                        >
                          <div>
                            {displayName}
                            <small className='ml-1 text-muted-foreground'>{info}</small>
                          </div>
                        </TableCell>
                        {onShowDetail && (
