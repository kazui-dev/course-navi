...
import { YearSelect, ClassSelect, TimetableCell, CourseSearch } from '@/components';
import { useSettingsStore, useCellStateStore, useTimetableStore } from '@/stores';
...
export default function Timetable({ onYearChange, onClassChange, onClearTimetable }: TimetableProps) {
  const currentYear = useSettingsStore(state => state.currentYear);
  const allCourses = useSettingsStore(state => state.allCourses);
  const availableYearsFromSectionTimes = useSettingsStore(state => state.availableYearsFromSectionTimes);
  ...
  const restoreTimetableSnapshot = useTimetableStore(state => state.restoreTimetableSnapshot);
  ...
  useEffect(() => {
    const handleUnregisterShortcut = (event: KeyboardEvent) => {
      ...
      if (!selectedCell || selectedCell.day === null || !selectedCell.period || selectedCell.period.length === 0) {
        return;
      }
    ...
    <div className="timetable">
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              <Button
                ...
                onClick={async () => {
                  try {
                    const applyHomeroom = useTimetableStore.getState().applyHomeroomCourses;
        ...
        <Table className="table-fixed">
          <TableHeader>
            <TableRow>
              ...
              {days.map((day, idx) => (
                <TableHead key={idx} className="text-center h-10">{day}</TableHead>
              ))}
            </TableRow>
          </TableHeader>
          <TableBody>
              ...
              return (
                <TableRow key={period}>
                  <TableCell className="text-center font-semibold bg-muted w-16 h-12 cursor-default">
                    <Tooltip>
                      ...
                      <TooltipContent side="left">
                        <p>{periodTimes[period]?.start ?? '-'} - {periodTimes[period]?.end ?? '-'}</p>
                      </TooltipContent>
                    </Tooltip>
                  </TableCell>
                  ...
                  {row.map((cell: TimetableCellContent, day: number) => {
                    ...
                    const nextCell = isEvenRow ? timetable[rowIdx + 1][day] : '';
                    const prevCell = !isEvenRow ? timetable[rowIdx - 1][day] : '';
      ...
      <div className="mt-4">
        <Button
          ...
          variant="destructive"
          onClick={async () => {
            const ok = await confirmService.confirm({ title: '消去の確認', message: '時間割の内容を消去しますか？', okLabel: '消去', cancelLabel: 'キャンセル' });
