...
import { buildClassName, parseClassName, validatePrerequisites } from "@/utils";
  ...
  useEffect(() => {
    ...
    const loadForYear = async () => {
      try {
        ...
        console.error(
          "Failed to load year-specific settings for selectedYear",
          error,
        );
      }
    };
    ...
    if (step === 2) {
      ...
      const inputsForStep3: TranscriptInput[] = selected.map((course) => {
        const acquiredSum = useTranscriptsStore
          .getState()
          .getAcquiredCredits(course.course, {
            upToYear: selectedYear ?? undefined,
        ...
        const remaining =
          maxCreditsVal === null
            ? null
            : Math.max(0, maxCreditsVal - acquiredSum);
        const defaultCredits =
          maxCreditsVal === null
            ? course.credits
            : Math.min(course.credits, remaining as number);
        return {
  ...
  const handleSubmit = async () => {
    try {
      ...
      inputs.forEach((input) => {
        ...
        const groupMembers =
          metadata &&
          Array.isArray(metadata.exclusive_group) &&
        ...
        <DialogFooter className="flex gap-2 justify-end">
          ...
          {step < 3 && (
            <Button
              ...
              disabled={
                isLoadingCourses ||
                (step === 1 && (!selectedClass || selectedYear == null))
              }
            >
              次へ
            </Button>
