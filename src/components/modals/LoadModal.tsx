...
import { useSaveSlotsStore, useTimetableStore, useCellStateStore, useSettingsStore } from '@/stores';
...
function LoadModal({ show, onHide }: LoadModalProps) {
  ...
  const loadTimetableFromSave = useTimetableStore(state => state.loadTimetableFromSave);
  ...
  const restoreDeletedSlot = useSaveSlotsStore(state => state.restoreDeletedSlot);
