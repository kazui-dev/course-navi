...
import { useSaveSlotsStore, useSettingsStore, useTimetableStore } from '@/stores';
...
function SaveModal({ show, onHide }: SaveModalProps) {
  ...
  const overwriteSaveSlot = useSaveSlotsStore(state => state.overwriteSaveSlot);
  const deleteSaveSlot = useSaveSlotsStore(state => state.deleteSaveSlot);
  const revertSlotToSnapshot = useSaveSlotsStore(state => state.revertSlotToSnapshot);
  ...
  const handleSave = async () => {
    ...
    if (result.success) {
      const savedSlot = useSaveSlotsStore.getState().allSaveSlots.find(slot => slot.name === trimmedName);
      setTableName('');
      setMemo('');
      setSaveError(null);
      onHide();
      ...
      const previousSnapshot = useSaveSlotsStore.getState().allSaveSlots.find(slot => slot.name === trimmedName) ?? null;
