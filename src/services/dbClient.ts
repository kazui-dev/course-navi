...
async function searchCourseByName(courseName: string, year: number): Promise<SearchResult[]> {
  ...
  const query = `
    ...
    ORDER BY st.day
  `;
  const rows = await db.select<{ day: number, periods_str: string, course_name: string }[]>(query, [courseName, year]);

  const result: SearchResult[] = rows.map(row => ({
    day: row.day,
    period: row.periods_str.split(',').map(Number).sort((a, b) => a - b),
...
async function fetchAllCourseNames(year: number): Promise<{ course: string, abbr: string, alias: string[] }[]> {
  const db = await getDbInstance();
  const query = `
    ...
    WHERE courses.year = ?
  `;
  const rows = await db.select<{ course: string, abbr: string, alias: string }[]>(query, [year]);
  return rows.map(r => {
    let aliasArr: string[] = [];
    if (r.alias) {
      try {
...
async function fetchHomeroomCourseData(sectionName: string, year: number): Promise<ProcessedRow[]> {
...
async function fetchCourseDataByCodes(codes: string[], year: number): Promise<ProcessedRow[][]> {
...
async function upsertUserProfile(year: number, department: string, division: string, classNum: string): Promise<void> {
...
async function insertSaveSlot(year: number, name: string, memo: string | null, timetable_codes_json: string): Promise<DbResult> {
  const db = await getDbInstance();
  const query = `
...
// 保存スロットの名前とメモを更新する。
// 名前重複時は `DUPLICATE_NAME` エラーを返す。
async function updateSaveSlotNameAndMemo(id: number, newName: string, newMemo: string | null): Promise<DbResult> {
  const db = await getDbInstance();
  const query = `
...
// 既存の保存スロットを上書きする。
async function overwriteSaveSlot(year: number, name: string, memo: string | null, timetable_codes_json: string): Promise<DbResult> {
  const db = await getDbInstance();
  const query = `
...
async function fetchPrerequisiteRules(courseName: string, year: number): Promise<PrerequisiteRuleRecord[]> {
  if (!courseName) {
    return [];
  }
  const db = await getDbInstance();
...
// 履修記録を一括挿入する。
async function insertTranscripts(records: NewTranscriptData[]): Promise<DbResult> {
  const db = await getDbInstance();
  const query = `
