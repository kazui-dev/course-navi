...
import { localBackup, type StoredUserProfile } from "@/services/localBackup";
...
const fetchYearResources = (year: number): Promise<YearResources> => {
  ...
  ]).then(([profile, allCourses, availableClasses]) => ({
    profile,
    allCourses,
    availableClasses,
  }));
...
const computeAllowedCourseSet = async (
  year: number,
  courseNames: string[],
): Promise<Set<string>> => {
...
export const useSettingsStore = create<SettingsStore>((set, get) => ({
    ...
    try {
      ...
      try {
        const profile = await Promise.all(
          transcriptsYears.map((y) => dbClient.fetchUserProfile(y)),
        );
        ...
        if (!anyExists) {
          ...
          if (backupAll && backupAll.length > 0) {
            try {
              ...
              console.error(
                "Failed to restore user profile from local backup:",
                err,
              );
            }
          }
      ...
      const allowedSet = await computeAllowedCourseSet(
        defaultYear,
        courseNames,
      );
      ...
      set({
        availableYearsFromSectionTimes:
          years.length > 0 ? years : [defaultYear],
        availableYearsForTranscripts:
          transcriptsYears.length > 0 ? transcriptsYears : [defaultYear],
  ...
  setCurrentClass: async (newClass) => {
    ...
    try {
      ...
      try {
        ...
        console.error(
          "Failed to recomputeAllowedCourses after class change",
          err,
        );
      }
  ...
  recomputeAllowedCourses: async (year) => {
    ...
    try {
      ...
      console.error(
        "Failed to fetch resources during recomputeAllowedCourses",
        err,
      );
      set({ isSettingsLoading: false });
    }
