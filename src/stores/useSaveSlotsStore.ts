...
type SaveSlotsActions = {
  ...
  saveTimetable: (
    ...
    currentYear: number,
    timetable: Timetable,
  ) => Promise<DbResult | ValidationResult>;
  overwriteSaveSlot: (
    name: string,
    memo: string,
    currentYear: number,
...
export const useSaveSlotsStore = create<SaveSlotsStore>((set, get) => {
  ...
  return {
          ...
          if (backup && backup.length > 0) {
            try {
              ...
              for (const s of backup) {
                try {
                  await dbClient.insertSaveSlot(
                    s.year,
                    s.name,
                    s.memo,
                    s.timetable_codes_json,
                  );
                } catch (innerErr) {
                  console.error(
                    "Failed to insert save slot from backup:",
                    innerErr,
                  );
                }
              }
              ...
              console.error(
                "Failed to restore save slots from local backup:",
                err,
              );
    ...
    saveTimetable: async (name, meta, currentYear, timetable) => {
      ...
      const clientValidation = validateSlotName(
        get().allSaveSlots,
        trimmedName,
      );
    ...
    updateSaveSlot: async (idToUpdate, newName, newMemo, currentYear) => {
      ...
      const validation = validateSlotName(
        get().allSaveSlots,
        trimmedName,
        idToUpdate,
      );
