...
import type { Timetable, TimetableCodesTable, CourseDataMap, CourseData, CourseListEntry, CellData } from '@/types';
...
interface TimetableActions {
  ...
  register: (data: CourseData[], currentCell: CellData | null, force?: boolean) => Promise<{
    success: boolean;
    blocked?: true;
    message?: string;
  }>;
  ...
  loadTimetableFromSave: (codesTable: TimetableCodesTable, year: number) => Promise<void>;
  ...
  getVisibleCoursesForCell: (cellKey: string, opts: { hideAcquired: boolean; filterPrereqs: boolean; allowedCourses?: Set<string> | null; isAllowedComputed?: boolean; currentYear?: number | null; }) => { subject: string; entries: CourseData[][] }[];
}
...
const focusCellAfterRegister = (data: CourseData[], currentCell: CellData | null) => {
 ...
 */
const _loadTimetableFromCodes = async (codesTable: TimetableCodesTable, year: number | null): Promise<Timetable> => {
  ...
  try {
    ...
    codesTable.forEach(row => row.forEach(code => {
      if (code) uniqueCodes.add(code);
    }));
    const codes = Array.from(uniqueCodes);
    if (codes.length > 0) {
      const courseDataLists = await dbClient.fetchCourseDataByCodes(codes, year);
...
export const useTimetableStore = create<TimetableStore>((set, get) => {
  ...
  const updateStateWithHistory = (entry: HistoryEntry, history: HistoryEntry[], historyIndex: number) => {
    const { table } = entry;
  ...
  const createInitialHistory = () => [createHistoryEntry(createEmptyTimetable())];
  ...
  return {
    ...
    _setTimetableAndHistory: (newTable, fromHistory = false) => {
      ...
      if (!fromHistory) {
        ...
        if (currentEntry && currentEntry.codesJson === newEntry.codesJson) return;
    ...
    register: async (data: CourseData[], currentCell: CellData | null, force = false): Promise<{ success: boolean; blocked?: true; message?: string }> => {
      ...
      try {
        ...
        if (violation && !force) {
          const formattedDescription = violation.description
            .replace(/ かつ /g, '\nかつ ')
            .replace(/ または /g, '\nまたは ');
          return { success: false, blocked: true, message: formattedDescription } as const;
        }
      } catch (error) {
      ...
      try {
        ...
        const courseMetadata = await dbClient.fetchCourseMetadata(registrationYear);
        const acquiredSum = useTranscriptsStore.getState().getAcquiredCredits(courseName, { upToYear: registrationYear, includeExclusiveGroup: true, includeSameYear: false, courseMetadata: courseMetadata });
        ...
        if (explicitMax !== null && explicitMax !== undefined) {
          ...
          if (acquiredSum + addCredits > maxCredits) {
            ...
            if (!force) {
              ...
              return { success: false, blocked: true, confirmType: 'maxCredits', message: `${acquiredSum}|${maxCredits}` } as any;
            }
          }
        }
      } catch (error) {
      ...
      data.forEach(courseData => {
        ...
        if (existingCourse !== '' && existingCourse.code !== courseCodeToRegister) {
    ...
    isCourseAtMax: (courseName: string, upToYear?: number) => {
      try {
        ...
        const metadataArray = Object.values(get().courseMetadataByName).filter(Boolean) as CourseMetadata[];
        const acquired = useTranscriptsStore.getState().getAcquiredCredits(courseName, { upToYear, includeExclusiveGroup: true, includeSameYear: false, courseMetadata: metadataArray });
    ...
    applyHomeroomCourses: async (className) => {
      ...
      try {
        const homeroomCourses = await dbClient.fetchHomeroomCourseData(className, year);
        ...
        try {
          ...
          console.error('Failed to apply homeroom courses in single history operation', err);
     ...
     */
    getVisibleCoursesForCell: (cellKey: string, opts: { hideAcquired: boolean; filterPrereqs: boolean; allowedCourses?: Set<string> | null; isAllowedComputed?: boolean; currentYear?: number | null; }) => {
      const { hideAcquired, filterPrereqs, allowedCourses, isAllowedComputed, currentYear } = opts;
      const codes = get().coursesByCellKey[cellKey] ?? [];
      if (codes.length === 0) return [] as { subject: string; entries: CourseData[][] }[];
      ...
      const groups = subjectOrder.reduce<{ subject: string; entries: CourseData[][] }[]>((acc, subject) => {
        const subjectCodes = subjectCourseCodes[subject] ?? [];
        const entries = subjectCodes.reduce<CourseData[][]>((collection, courseCode) => {
          ...
          if (hideAcquired && get().isCourseAtMax(courseName, currentYear ?? undefined)) {
            return collection;
          }
